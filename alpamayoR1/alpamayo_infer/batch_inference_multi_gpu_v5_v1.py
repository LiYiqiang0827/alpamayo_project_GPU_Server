#!/usr/bin/env python3
"""
多GPU批量Clip推理 V5.1 - 输入双缓冲优化版
在V5基础上添加：
1. 输入双缓冲：GPU计算和数据传输完全重叠
2. 双CUDA Stream：compute_stream用于推理，transfer_stream用于数据传输
3. 预分配GPU内存，避免动态分配

用法:
    python3 batch_inference_multi_gpu_v5_v1.py --chunk 0 --num_frames 0 --gpus 1,2,3,5,6,7 --batch_size 8
"""
...
def main():
    parser = argparse.ArgumentParser(
        description="多GPU批量Clip推理 V5.1 - 输入双缓冲优化版"
    )
    ...
    print(f"======================================================================")
    print(f"🚀 多GPU批量Clip推理 V5.1 - 输入双缓冲优化版")
    print(f"======================================================================")
    print(
        f"配置: chunk={args.chunk}, {args.num_frames if args.num_frames > 0 else '全部'}帧/clip, step={args.step}"
    )
    ...
    summary_path = (
        f"/data01/vla/cot_collection/batch_multi_gpu_v51_chunk{args.chunk}.json"
    )
