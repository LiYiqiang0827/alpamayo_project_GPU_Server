...
def create_trajectory_plot(infer_idx, result_dir, data_dir):
    ...
    for i in range(pred.shape[0]):
        ...
        ax.plot(pred_xy_rot[0], pred_xy_rot[1], "o-", color=colors[i], label=f"Pred #{i+1}", markersize=2, linewidth=1.5)
...
def main():
    ...
    for i in tqdm(range(len(merged))):
        ...
        try:
            ...
            frame.save(f'{FRAME_OUTPUT_DIR}/frame_{row["infer_idx"]:06d}.png', quality=95)
