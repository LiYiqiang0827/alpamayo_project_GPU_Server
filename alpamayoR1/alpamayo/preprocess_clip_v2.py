...
def preprocess_clip_v2(clip_id, chunk=0):
    ...
    FUTURE_STEPS = 64
    JPG_QUALITY = 95
    MAX_TIME_DIFF_MS = 50  # 最大时间差 50ms
    
    print(f'=== 预处理 {clip_id} (正确版本) ===\n')
    ...
    for ego_idx in range(HISTORY_STEPS, len(ego_df) - FUTURE_STEPS):
        ...
        for cam_name, cam_prefix in cameras:
            ...
            # 找最接近 ego_ts 的帧 (相机帧频率更高，取 <= ego_ts 的最新帧)
        ...
        if valid:
            valid_frames.append({
                ...
                'ego_idx': ego_idx,
                't0_timestamp': ego_ts,
                **{f'{cam_prefix}_f0': f'camera_images/{cam_name}/{cam_frames[cam_name]["frame_idx"]:06d}.jpg' for cam_name, cam_prefix in cameras},
                **{f'{cam_prefix}_f1': f'camera_images/{cam_name}/{cam_frames[cam_name]["frame_idx"]-1:06d}.jpg' for cam_name, cam_prefix in cameras},
                **{f'{cam_prefix}_f2': f'camera_images/{cam_name}/{cam_frames[cam_name]["frame_idx"]-2:06d}.jpg' for cam_name, cam_prefix in cameras},
                **{f'{cam_prefix}_f3': f'camera_images/{cam_name}/{cam_frames[cam_name]["frame_idx"]-3:06d}.jpg' for cam_name, cam_prefix in cameras},
            })
    ...
    # Step 4: 保存egomotion数据 (世界坐标和局部坐标)
    print('\nStep 4: 保存 egomotion 数据...')
