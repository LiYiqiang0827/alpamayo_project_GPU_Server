...
CLIP_ID = 'e9162b7f-00f1-4563-890e-15c9fc89ad7a'
...
# 测试不同位置
test_positions = [0, 100, 500, 1000, 1500, 1900]
print('推理不同位置...')
results = []
for pos in test_positions:
    ...
    if isinstance(coc, np.ndarray):
        coc = coc.tolist()
    coc_str = str(coc[0])[:40] if coc and len(coc) > 0 else "N/A"
    
    results.append(ade)
    status = "✅" if ade < 1.5 else ("⚠️" if ade < 3.0 else "❌")
    print(f"{status} 位置{pos} (infer_idx={infer_idx}): ADE={ade:.3f}m - {coc_str}...")
...
print('\n对比：')
print('  Clip 1 (简单直行): ADE≈0.8m')
print('  Clip 2 (复杂路口): ADE≈25m')
print(f'  本clip (纯直道): ADE≈{np.mean(results):.1f}m')
