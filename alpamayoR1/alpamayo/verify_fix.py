...
for pos in test_positions:
    ...
    results.append({'pos': pos, 'infer_idx': infer_idx, 'ade': ade})
    print(f"   位置{pos} (infer_idx={infer_idx}): ADE={ade:.3f}m")

print('\n=== 验证结果 ===')
print('最早正确参考值: Frame 138=0.80m, 139=1.02m, 140=1.04m')
for r in results:
    print(f"当前结果: 位置{r['pos']} (infer_idx={r['infer_idx']}) ADE={r['ade']:.3f}m ✅" if r['ade'] < 1.5 else f"当前结果: 位置{r['pos']} (infer_idx={r['infer_idx']}) ADE={r['ade']:.3f}m ⚠️")
