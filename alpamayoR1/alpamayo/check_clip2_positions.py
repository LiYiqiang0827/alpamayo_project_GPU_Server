...
# 测试不同位置
test_positions = [0, 50, 100, 200, 400]
print('推理不同位置...')
for pos in test_positions:
    ...
    if isinstance(coc, np.ndarray):
        coc = coc.tolist()
    coc_str = str(coc[0]) if coc and len(coc) > 0 else "N/A"
    
    status = "✅" if ade < 1.5 else ("⚠️" if ade < 3.0 else "❌")
    print(f"{status} 位置{pos} (infer_idx={infer_idx}): ADE={ade:.3f}m - {coc_str[:60]}...")
