    ...
    def sample_trajectories_from_data_with_vlm_rollout(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            return_diffusion_steps: If True, return all intermediate diffusion steps.
        ...
        Returns:
            ...
            (If return_diffusion_steps=True) extra dict contains diffusion_steps_xyz, diffusion_steps_action, diffusion_steps_time.
        """
        ...
        def step_fn(
            ...
            expert_attn_impl = getattr(self.expert.config, '_attn_implementation_internal', None)
            use_flash_attn = (expert_attn_impl == "flash_attention_2")
            if use_flash_attn:
        ...
        # Call diffusion sampling, optionally returning all intermediate steps
        if return_diffusion_steps:
            all_actions, diffusion_time_steps = self.diffusion.sample(
            ...
            # all_actions shape: (total_batch, num_steps+1, 64, 2)
            # diffusion_time_steps shape: (num_steps+1,)
        else:
            all_actions = self.diffusion.sample(
                batch_size=total_batch,
                step_fn=step_fn,
                device=device,
        ...
        if return_diffusion_steps:
            # Convert all intermediate steps to trajectories
            # all_actions: (total_batch, num_steps+1, 64, 2)
            num_steps = all_actions.shape[1]
            all_trajectories = []
            for step_idx in range(num_steps):
                step_action = all_actions[:, step_idx]  # (total_batch, 64, 2)
                step_traj, _ = self.action_space.action_to_traj(
                    step_action, hist_xyz_rep, hist_rot_rep
                )  # (total_batch, 64, 3)
                all_trajectories.append(step_traj[..., :2])  # Only keep x, y
            ...
            pred_xyz, pred_rot = self.action_space.action_to_traj(
                final_action, hist_xyz_rep, hist_rot_rep
            )
        else:
            pred_xyz, pred_rot = self.action_space.action_to_traj(
                all_actions, hist_xyz_rep, hist_rot_rep
            )
            all_steps_xyz = None
        ...
        if kwargs.get("return_extra", False) or return_diffusion_steps:
            ...
            if return_diffusion_steps and all_steps_xyz is not None:
                ...
                extra["diffusion_steps_time"] = diffusion_time_steps.cpu().numpy()
